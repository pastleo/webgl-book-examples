...
const ALTITUDE_PACKING_OFFSET = 128;
const PACKED_ALTITUDE_R_ZERO = ALTITUDE_PACKING_OFFSET - 1;
...
const landMapFragmentShaderSource = `
precision highp float;
...
vec3 packFloatToRgb(float value) {
  value += ${ALTITUDE_PACKING_OFFSET.toFixed(2)};
  float r = floor(value);
  value -= r;
...
float unpackRgbToFloat(vec3 rgb) {
  ...
  return value - ${ALTITUDE_PACKING_OFFSET.toFixed(2)};
}
...
function createWelcomeCanvas() {
  ...
  ctx.fillText('(WebGL1 版本)', canvas.width / 2, secondBaseLine + secondLineHeight * 2);
...
function checkSailboatCollision(app) {
  ...
  if (!app.sailboatAltitudes) {
    app.sailboatAltitudes = new Uint8Array(width * height * 2 * dataStep);
  }
  ...
  for (let i = 0, len = width * height * dataStep; i < len; i += dataStep) {
    if (app.sailboatAltitudes[i] > PACKED_ALTITUDE_R_ZERO) return true;
  }
  return false;
}
...
function initGame(app) {
  ...
  [
    ...
    element.addEventListener('touchstart', () => {
      pressDirection(app, dir);
    });
    element.addEventListener('touchend', () => {
      releaseDirection(app, dir);
    });
