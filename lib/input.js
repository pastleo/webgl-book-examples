 ...
 * @property {[number, number]} cameraRotationXY
 * @property {number} cameraDistance
 * @property {[number, number, number]} cameraViewing
 */
 ...
 * @property {[number, number]} [mouseCoord]
 * @property {boolean} [mousemove]
 * @property {boolean} [touched]
 * @property {[number, number]} [touchCoord]
 * @property {boolean} [touchmove]
 * @property {number} [pitchSq]
 * @property {boolean} [touchpadDetected]
 * @property {number} [wheelMinDeltaY]
 * @property {number} [gestureRotation]
 * @property {number} [gestureScale]
 */

const CAMERA_MAX_ROTATION_X = degToRad(-0.5);
const CAMERA_MAX_ROTATION_X_FREE = degToRad(90);
const CAMERA_MIN_ROTATION_X = degToRad(-90);

function moveCameraAngle(state, angles, maxRotationX) {
  state.cameraRotationXY[0] += angles[0];
  state.cameraRotationXY[1] += angles[1];

  if (state.cameraRotationXY[0] > maxRotationX) {
    state.cameraRotationXY[0] = maxRotationX;
 ...
 * @returns {Input}
 */
export default function listenToInputs(canvas, state, freeCamera = false) {
  /**
   * @type Input
   */
  const input = { keyboard: {} };
  const maxRotationX = freeCamera ? CAMERA_MAX_ROTATION_X_FREE : CAMERA_MAX_ROTATION_X;
    ...
    if (input.mousedown) {
      ...
      if (
        ...
        if (input.mousedown === 'left') {
          ...
          moveCameraAngle(state, [(preOffsetY - offsetY) / 100, (preOffsetX - offsetX) / 100], maxRotationX)
        }
  ...
  canvas.addEventListener('wheel', event => {
    ...
    moveCameraAngle(state, angleDelta, maxRotationX);
  })
