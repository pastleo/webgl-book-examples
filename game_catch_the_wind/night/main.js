...
const SAILBOAT_COLLISION_BORDERS = [0.5, 0.5, 2.75, 1.4]; // [+x, -x, +z, -z]
...
vec2 skyCoordToLatLong(vec3 skyCoord) {
  return degrees(vec2(
    ...
    acos(skyCoord.y)
  )) * vec2(1, -1) + vec2(skyCoord.x > 0.0 ? 90.0 : -90.0, 90);
}

mat3 xRotate(float angleInRadians) {
...
float hash0(vec2 p) {
  return fract(sin(mod(dot(p, vec2(83, 89)), radians(180.0))) * u_seed);
}
float hash1(vec2 p) {
  return fract(sin(mod(dot(p, vec2(71, 73)), radians(180.0))) * u_seed);
}
float hash2(vec2 p) {
...
function renderSailboat(app, viewMatrix, programInfo) {
  ...
  const worldMatrix = matrix4.multiply(
    ...
    matrix4.translate(0, Math.sin(time * 0.0017) * 0.05 + state.sailboatTranslateY, 0),
  );
...
function renderLand(app, viewMatrix, programInfo) {
  ...
  twgl.setUniforms(programInfo, {
    ...
    u_worldMatrix: worldMatrix,
    u_normalMatrix: matrix4.transpose(matrix4.inverse(worldMatrix)),
    u_diffuse: [0.431372, 0.380392, 0.294117],
...
function renderSkybox(app, projectionMatrix, inversedCameraMatrix) {
  ...
  twgl.setUniforms(programInfos.skybox, {
    ...
    u_seed: app.state.seed,
    u_time: app.time,
    u_sailDistance: -app.state.sailboatLocation[1],
  });
...
function checkSailboatCollision(app) {
  ...
  const sailboatLocInLandMap = [
    state.sailboatLocation[0] - landMapOffset[0],
    state.sailboatLocation[1] - landMapOffset[1],
  ];

  const left = Math.floor(
    (
      (sailboatLocInLandMap[0] - SAILBOAT_COLLISION_BORDERS[1]) /
        LAND_MAP_SIZE[0] * 1 + 0.5
    ) * framebuffers.landMap.width
  );
  const right = Math.ceil(
    (
      (sailboatLocInLandMap[0] + SAILBOAT_COLLISION_BORDERS[0]) /
        LAND_MAP_SIZE[0] * 1 + 0.5
    ) * framebuffers.landMap.width
  );
  const top = Math.floor(
    (
      (sailboatLocInLandMap[1] + SAILBOAT_COLLISION_BORDERS[2]) /
        LAND_MAP_SIZE[1] * -1 + 0.5
    ) * framebuffers.landMap.height
  );
  const bottom = Math.ceil(
    (
      (sailboatLocInLandMap[1] - SAILBOAT_COLLISION_BORDERS[3]) /
        LAND_MAP_SIZE[1] * -1 + 0.5
    ) * framebuffers.landMap.height
  );
  ...
  if (!app.sailboatAltitudes) {
    app.sailboatAltitudes = new Int32Array(width * height * 2 * dataStep);
  }

  gl.readPixels(
    left, top, width, height,
    format, type,
    app.sailboatAltitudes,
  );

  for (let i = 0, len = width * height * dataStep; i < len; i += dataStep) {
    if (app.sailboatAltitudes[i] > 0) return true;
  }
  return false;
}
...
function gameUpdate(app, timeDiff, now) {
  ...
  if (state.started) {
    if (!state.gameOver) {
      ...
      state.sailboatTranslateY = Math.max(state.sailboatTranslateY - timeDiff * 0.00005, -1);
    }
...
async function main() {
  ...
  try {
    ...
    initGame(app);
    if (FREE_CAMERA) {
      app.input = listenToInputs(app.gl.canvas, app.state, FREE_CAMERA);
    }
