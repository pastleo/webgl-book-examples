...
vec3 starsLayer(vec2 uv) {
  ...
  for(int i = 0; i <= 1; i++) {
    for(int j = 0; j <= 1; j++) {
      ...
      float rnd = hash21(subId + 0.3);
      vec3 starColor = sin(vec3(1.4, 1.5, 1.6) * rnd * 0.0) * 0.5 + 0.5;
      color += starColor * star(gv - offset, rnd);
    }
  }
  return color;
}
void main() {
  vec3 cubeCoord = normalize(v_normal);
  vec3 color = vec3(0);
  ...
  outColor = vec4(color + vec3(0.0588, 0.0431, 0.211), 1);
