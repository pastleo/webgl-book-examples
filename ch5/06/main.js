...
const groundFragmentShaderSource = `#version 300 es
precision highp float;
...
void main() {
  vec2 texcoord = (v_mirrorTexcoord.xy / v_mirrorTexcoord.w) * 0.5 + 0.5;
  vec3 normal = texture(u_normalMap, v_texcoord).xyz * 2.0 - 1.0;
...
async function setup() {
  ...
  const groundProgramInfo = twgl.createProgramInfo(gl, [vertexShaderSource, groundFragmentShaderSource]);
...
function renderGround(app, viewMatrix, mirrorViewMatrix, programInfo) {
  ...
  twgl.setUniforms(programInfo, {
    ...
    u_worldMatrix: worldMatrix,
    u_normalMatrix: matrix4.transpose(matrix4.inverse(worldMatrix)),
    u_normalMap: textures.groundNormal,
    u_diffuse: [0.75, 0.75, 0.75],
...
function render(app) {
  ...
  gl.useProgram(groundProgramInfo.program);
  twgl.setUniforms(groundProgramInfo, globalUniforms);
  renderGround(app, viewMatrix, mirrorViewMatrix, groundProgramInfo);
}
